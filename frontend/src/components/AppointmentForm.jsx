...
const AppointmentForm = () => {
  ...
  const [doctorId, setDoctorId] = useState("");  // Use doctor ID
  ...
  const handleAppointment = async (e) => {
    ...
    try {
      ...
      console.error("Error submitting appointment:", error); // Debug log
      toast.error(error.response?.data?.message || "Error submitting appointment");
    }
  };

  // Handle doctor selection by ID
  const handleDoctorSelection = (id) => {
    ...
    if (selectedDoctor) {
      ...
      setDoctorId(id);  // Save the selected doctor's ID
    }
  };
